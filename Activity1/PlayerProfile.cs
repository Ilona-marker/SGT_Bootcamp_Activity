...
namespace CSharp.Activity.Profile
{
    ...
    public class PlayerProfile
    {
        ...
        public int ComputeAge()
        {
            ...
            // :::
            /*My solution
             tempAge = ((currentYear - birthdayYear) * 365 + (currentMonth - birthdayMonth) * 31 + (currentDayOfMonth - birthdayDay)) / 365;
            */


            // we only process if currnt year is after birth
            if (currentYear >= birthdayYear)
                //generally,age is the difference in year numbers
                tempAge = currentYear - birthdayYear;
            //but the birthday day has not arrived yet this year
            //then we need to subtruct one from age
            if (currentMonth < birthdayMonth || 
               ( currentMonth == birthdayMonth && currentDayOfMonth < birthdayDay))
            {
                tempAge--;
            }
